local filename='data/expire.lua'
 ...
 local function cron()
   for date, values in pairs(cronned) do
     if date < os.time() then ótime's up
       send_msg(values[1][1], "„œ  “„«‰ ê—ÊÂ ‘„« »Â « „«„ —”Ìœ"..values[1][2], ok_cb, false)
       delete_cron(date) 
   end
 ...
 local function actually_run(msg, delay,text)
   ...
   return "„œ  “„«‰ «‰ﬁ÷«Ì ê—ÊÂ »Â" .. os.date("%x at %H:%M:%S",delay) .. "  ‰ŸÌ„ ‘œ \n\n Ê÷ÌÕ« \n" .. text .. "'"
 end
 ...
 return {
   ...
   patterns = {
     "^[!/](expire) ([0-9]+[hmsdHMSD]) (.+)$",   ó- e.g : for a month enter : 720hms - then , in text enter gp id and admin id 
